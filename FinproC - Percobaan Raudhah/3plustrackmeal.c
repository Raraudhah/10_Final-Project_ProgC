...
void dashboard(const char *email, int targetKalori) {
    ...
    while (1) {
        ...
        sprintf(exerciseFile, "logs/%s_exercise_logs.txt", email);
        FILE *fex = fopen(exerciseFile, "r");
        if (fex) {
            char line[MAX_LINE];
...
void showMealHistory(const char *email) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%s %s %s %s %s %d",
                   user, tanggal, waktu, mealType, food, &kalori) == 6) {
            if (strcmp(user, email) == 0 && isWithin7Days(tanggal)) {
                found = 1;
                printf("%-11s %-9s %-9s %-15s %d kkal\n", tanggal, waktu, mealType, food, kalori);
            }
        }
    }
    fclose(f);
