...
void showWeeklySummary(const char *email, int targetKalori) {
    printf("\n=== RINGKASAN KALORI 7 HARI TERAKHIR ===\n");
    printf("+------------+--------------+----------------+----------------+\n");
    printf("|  Tanggal   | Kalori Meal  | Kalori Exercise|  Kalori Netto  |\n");
    printf("+------------+--------------+----------------+----------------+\n");

    time_t now = time(NULL);  // hanya dideklarasi sekali di sini
    ...
    for (int i = 6; i >= 0; i--) {
        ...
        // Hitung total kalori meal dari calories_logs.txt
        FILE *fmeal = fopen("logs/calories_logs.txt", "r");
        if (fmeal) {
            char line[MAX_LINE];
            while (fgets(line, sizeof(line), fmeal)) {
        ...
        // Hitung total kalori exercise dari [email]_exercise_logs.txt
        char exFile[100];
    ...
    if (daysWithData > 0) {
        ...
        printf("\n--- Ringkasan Mingguan ---\n");
        printf("Rata-rata kalori masuk  : %.0f kkal/hari\n", avgMeal);
        printf("Rata-rata kalori keluar : %.0f kkal/hari\n", avgEx);
        printf("Kalori bersih rata-rata : %.0f kkal/hari\n", netto);
        printf("Target kalori harianmu  : %d kkal\n", targetKalori);

        printf("\n--- Analisis dan Saran ---\n");

        if (netto > targetKalori + 200) {
            printf("Terjadi surplus kalori yang cukup besar dari targetmu.\n");
            printf("Perhatikan porsi makan dan pertimbangkan tambah aktivitas fisik.\n");
        } else if (netto < targetKalori - 200) {
            printf("Kamu cenderung defisit kalori.\n");
            printf("Jika tujuanmu bukan diet, pastikan kebutuhan energi tetap terpenuhi.\n");
        } else {
            printf("Kalori bersihmu cukup stabil dibanding target. Pertahankan pola hidupmu! 💪\n");
        }

        if (avgEx < 150) {
            printf("Aktivitas fisikmu rendah. Tambahkan jalan kaki, stretching, atau olahraga ringan 3x seminggu.\n");
        } else if (avgEx > 400) {
            printf("Aktivitasmu sangat baik minggu ini. Kamu aktif dan menjaga tubuh tetap fit!\n");
        } else {
            printf("Aktivitasmu cukup. Tapi jangan lupa terus jaga konsistensi.\n");
        }
    } else {
        printf("\nBelum ada cukup data minggu ini untuk dianalisis.\n");
    }
}
...
void dailyMealMenu(const char *email) {
    ...
    while (1) {
        ...
        if (pilih == 1) {
            showMealHistory(email);  // ⬅ ganti dari printMealHistory per meal
        } else if (pilih == 2) {
            // Submenu seperti biasa
            printf("\nPilih jenis makanannya:\n");
            printf("1. Breakfast\n2. Lunch\n3. Dinner\nPilihan: ");
            int mealTypeChoice;
...
void showMealHistory(const char *email) {
    ...
    printf("\n--- Meal History 7 Hari Terakhir ---\n");
    printf("Tanggal     Waktu     Jenis     Makanan                      Kalori\n");
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        // Cari spasi terakhir → kemungkinan kalori
        char *lastSpace = strrchr(afterMealType, ' ');
        if (!lastSpace) continue;

        kalori = atoi(lastSpace + 1); // ambil angka di belakang
        *lastSpace = '\0'; // potong angka → sisanya nama makanan
        strcpy(makanan, afterMealType);
    ...
    if (!found) {
        printf("Tidak ada data makanan dalam 7 hari terakhir.\n");
    }
}
